...
## 풀이 순서
#### 1) rock, scissors, paper를 담을 공간을 만든다(배열) => let storage;
#### 2) 0일땐 별다른 규칙이 없어보이므로 다음 1을 위해 결과값을 담는 공간(storage)에 빈배열 3개를 push해준다. => [[], [], []]
#### 3) 1일때도 2,3...과 같은 규칙이 없어보이므로 0에서 만들어준 각각의 빈배열에 rock, scissors, paper를 push해준다. => [['rock'], ['sissors'], ['paper']]
#### 4) 2부터는 공통 규칙이 보인다.
...
5. 첫번째로 'rock' 요소에 접근한다. 그고 현재 첫번째 인자로 들어온 ['rock']에 연결시켜준다. 그리고 그것을 재귀함수의 첫번째 인자로 넣고, 두번째 인자 숫자에 + 1을 해주고 재귀함수를 실행한다.
